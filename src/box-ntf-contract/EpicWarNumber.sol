...
interface IEpicWarBox {
  function setRandomNumber(uint16 eventId, uint256 randomness) external;
}

contract EpicWarNumber is
  VRFConsumerBase,
  ...
  AccessControl
{
  ...
  mapping(bytes32 => uint16) private reqToEvent;
  mapping(address => uint256) private eventToNumber;

  event RequestRandomNumber(bytes32 indexed requestId, uint16 indexed eventId);
  ...
  function getEventRandomNumber(uint16 _eventId)
    external
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness)
    ...
    override
  {
    uint16 eventId = reqToEvent[requestId];
  ...
  function setBoxContract(address _boxContract)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(
      _boxContract != address(0) && _boxContract != msg.sender,
      "invalid address"
    );
