...
contract NFTWEscrow is Initializable, OwnableUpgradeable, ERC165Upgradeable, INFTEscrow, AccessControlUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    address private EPIC_ERC20_ADDR;
    INFTEpicWar private EPIC_NFT_ADDR;
    address private signer;
    bytes32 private constant OWNER_ROLE = keccak256("OWNER_ROLE");
    bytes32 public constant WITHDRAW_NFT_WITH_SIG_TYPEHASH = keccak256("NftWithdrawal(uint256[] tokenIds,uint256 nonce,uint256 deadline)");
    bytes32 public constant WITHDRAW_TOKEN_WITH_SIG_TYPEHASH = keccak256("TokenWithdrawal(uint256 amount,uint256 nonce,uint256 deadline)");
    ...
    function depositNft(uint[] calldata tokenIds) 
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            { 
                ...
                require(IERC721Upgradeable(EPIC_NFT_ADDR).ownerOf(tokenId) == _msgSender(), "E2"); // E3: Not your token
                IERC721Upgradeable(EPIC_NFT_ADDR).safeTransferFrom(_msgSender(), address(this), tokenId);  
                NftInfo[tokenId] = NFTInfo(_msgSender(), true);
                emit NFTDeposited(tokenId, _msgSender());
            }
        }
    ...
    function nftWithdrawal(uint256[] calldata tokenIds ,EIP712Signature memory _signature) external nonReentrant{
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            emit NFTWithdrawed(tokenId, _msgSender()); // EPIC NFT `id` Withdrawed to `address`
        }       

    }


    function tokenWithdrawal(uint256 amount ,EIP712Signature calldata _signature) external nonReentrant{
