    ...
    ERC721HolderUpgradeable
{
    ...
    bytes32 public constant WITHDRAW_NFT_WITH_SIG_TYPEHASH = keccak256(
    ...
    function __NFTWEscrow_init(address _token, address _nft)
        public
        initializer
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            {
                ...
                require(
                    IERC721Upgradeable(EPIC_NFT_ADDR).ownerOf(tokenId) ==
                        _msgSender(),
                    "E2"
                ); // E3: Not your token
    ...
    function nftWithdrawal(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (EPIC_NFT_ADDR.checkExistsToken(tokenId)) {
                ...
                EPIC_NFT_ADDR.safeTransferFrom(
                    address(this),
                    _msgSender(),
                    tokenId
                );
